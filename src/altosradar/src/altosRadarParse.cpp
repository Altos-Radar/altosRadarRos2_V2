...
float rcsCal(float range, float azi, float snr, float* rcsBuf) {
    ...
    float rcs = std::pow(range, 2.6) * snr / 5.0e6 / rcsBuf[ind];
...
float hist(std::vector<POINTCLOUD> pointCloudVec, float* histBuf, float step) {
    int ind = 0;
    float vr = 0;

    for (std::size_t i = 0; i < pointCloudVec.size(); i++) {
        for (std::size_t j = 0; j < 30; j++) {
            if (abs(pointCloudVec[i].point[j].range) > 0) {
                vr = pointCloudVec[i].point[j].doppler /
                     cos(pointCloudVec[i].point[j].azi);
                ind = (vr - vrMin) / step;
                if (vr > 60 || vr < -60 || std::isnan(vr)) {
                    continue;
                }
                if (vr <= 0) {
                    histBuf[ind]++;
    ...
    return float(
               (std::max_element(histBuf, histBuf + (int((vrMax - vrMin) / step))) -
                histBuf)) *
               step +
           vrMin;
}

void calPoint(std::vector<POINTCLOUD> pointCloudVec,
              pcl::PointCloud<pcl::PointXYZHSV>::Ptr cloud,
              int installFlag,
              float* rcsBuf,
              float step,
              float* histBuf,
    ...
    for (std::size_t i = 0; i < pointCloudVec.size(); i++) {
        for (std::size_t j = 0; int(j) < pointNumPerPack; j++) {
            if (abs(pointCloudVec[i].point[j].range) > 0) {
...
int main(int argc, char** argv) {
    ...
    std::vector<POINTCLOUD> pointCloudVec;
    ...
    unsigned char mode;
    float* histBuf =
        (float*)malloc(sizeof(float) * int((vrMax - vrMin) / vStep));
    ...
    while (rclcpp::ok()) {
        ...
        if (ret > 0) {
            ...
            if ((mode == 1 && (static_cast<int>(curObjInd) + 1) * pointNumPerPack >=
                                  static_cast<int>(pointCloudBuf.pckHeader.objectCount))) {
                // if (pointCloudVec.size() * pointNumPerPack < cntPointCloud[0]
                // + cntPointCloud[1]) {
                //     printf(
                //         "FrameId %d %ld Loss %ld pack(s) in %d "
                //         "packs------------------------\n",
                //         pointCloudBuf.pckHeader.frameId,
                //         pointCloudVec.size(), int(ceil(cntPointCloud[0] /
                //         pointNumPerPack) +
